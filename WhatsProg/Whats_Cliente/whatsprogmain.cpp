...
DWORD WINAPI le_msg(LPVOID lpParameter)
{
  ...
  while (s.connected()) {
    conexaoOK = (s.read_int(CMD_IN) != SOCKET_ERROR);
    ...
    if(conexaoOK)
    {
        ...
        case (CMD_NOVA_MSG):{
            ...
            if(conexaoOK)conexaoOK = (s.read_int(id) != SOCKET_ERROR);

            if(conexaoOK) conexaoOK = (s.read_string(remetente) != SOCKET_ERROR);

            if(conexaoOK) conexaoOK = (s.read_string(texto) != SOCKET_ERROR);
            Mensagem M;
            ...
            if(conexaoOK)
            {
                bool novaconversa = true;

                for(int i = 0; i < DCliente.size() ; i ++)
                {
                   if(DCliente[i].getCorrespondente() == remetente)
                   {
                        DCliente[i].insertMessage(M);
                        novaconversa = false;
                   }
                }
                if(novaconversa == true)
                {
                    DCliente.insertConversa(remetente);
        ...
        case (CMD_MSG_RECEBIDA):
            if(conexaoOK)s.read_int(id);
            for(unsigned i = 0; i < DCliente.size(); i++)
                for(unsigned k = 0; k < DCliente[i].size(); k++)
                    if(DCliente[i][k].getId() == id && DCliente[i][k].getStatus() == MSG_ENVIADA)
                        DCliente[i][k].setStatus(MSG_RECEBIDA);
            break;
        case (CMD_MSG_ENTREGUE):
            if(conexaoOK)s.read_int(id);
            for(unsigned i = 0; i < DCliente.size(); i++)
                for(unsigned k = 0; k < DCliente[i].size(); k++)
                    if(DCliente[i][k].getId() == id)
                        if(DCliente[i][k].getRemetente() == DCliente.getMeuUsuario())
                            if(DCliente[i][k].getStatus() == MSG_ENVIADA || DCliente[i][k].getStatus() == MSG_RECEBIDA)
                                DCliente[i][k].setStatus(MSG_ENTREGUE);
            break;
        case (CMD_MSG_LIDA2):{
            if(conexaoOK)conexaoOK=s.read_int(id);
            if(conexaoOK)
            {
                for(unsigned i = 0; i < DCliente.size(); i++)
                    for(unsigned k = 0; k < DCliente[i].size(); k++)
                        if(DCliente[i][k].getId() == id)
                            if(DCliente[i][k].getRemetente() == DCliente.getMeuUsuario())
                                if(DCliente[i][k].getStatus() == MSG_ENVIADA || DCliente[i][k].getStatus() == MSG_RECEBIDA || DCliente[i][k].getStatus() == MSG_ENTREGUE)
                                    DCliente[i][k].setStatus(MSG_LIDA);
            }
            ...
            if(conexaoOK)
            {
                for(unsigned i = 0; i < DCliente.size(); i++)
                    for(unsigned k = 0; k < DCliente[i].size(); k++)
                        if(DCliente[i][k].getId() == id)
                            if(DCliente[i][k].getRemetente() == DCliente.getMeuUsuario())
                                if(DCliente[i][k].getStatus() == MSG_ENVIADA)
                                    DCliente.eraseConversa(i);
    ...
    ui(new Ui::WhatsProgMain)
{
    ...
    // A caixa de dialogo de nova conversa
    NewContato = new NewContatoDialog(this);
    ...
    connect(NewContato, SIGNAL(NovoContato(string)),this,SLOT(slotNovaConversa(string)));
}
...
void WhatsProgMain::slotNovaConversa(const string &Dest)
{
